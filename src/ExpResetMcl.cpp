...
bool ExpResetMcl::sensorUpdate(double lidar_x, double lidar_y, double lidar_t, bool inv, sensor_msgs::LaserScan &mode_scan, std_msgs::UInt8 &mode_scan_pattern)
{
  ...
  {
    generateScanPattern(scan, pra);
    init_random_scan_angle_flag = false;
  }

  findModeScanPattern(mode_scan, mode_scan_pattern, pra);
...
void ExpResetMcl::generateScanPattern(Scan scan, Particle &pra){
  int angle_num = 5;
...
void ExpResetMcl::findModeScanPattern(sensor_msgs::LaserScan &mode_scan, std_msgs::UInt8 &mode_scan_pattern, Particle &pra){
